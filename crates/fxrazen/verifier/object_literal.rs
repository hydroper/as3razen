...
impl ObjectLiteralSubverifier {
    pub fn verify_object_initializer(verifier: &mut Subverifier, initializer: &ObjectInitializer, context: &VerifierExpressionContext) -> Result<Option<Thingy>, DeferError> {
    ...
    fn verify_object_initializer_for_options_class(verifier: &mut Subverifier, initializer: &ObjectInitializer, options_class: &Thingy) -> Result<(), DeferError> {
        let mut missing = HashSet::<Thingy>::new();
        for (_, thingy) in options_class.prototype(&verifier.host).borrow().iter() {
            if thingy.is::<VariableSlot>() && !thingy.is_opt_variable_for_options_class(&verifier.host)? {
                thingy.static_type(&verifier.host).defer()?;
                missing.insert(thingy.clone());
            }
        }
                    ...
                    if let Some(name) = field.shorthand() {
                        ...
                        if let Some(short_ref_1) = short_ref.as_ref() {
                            if let Some(variable) = variable.as_ref() {
                                ...
                                let coercion = TypeConversions(&verifier.host).implicit(short_ref_1, &variable_data_type, false)?;
                                let Some(coercion) = coercion else {
                                    verifier.add_verify_error(&name.location, FxDiagnosticKind::ImplicitCoercionToUnrelatedType, diagarg![short_ref_1.static_type(&verifier.host), variable_data_type]);
        ...
        for m in &missing {
            verifier.add_verify_error(&initializer.location, FxDiagnosticKind::MustSpecifyOption, diagarg![m.name().to_string()]);
        }

        Ok(())
    }

    fn verify_non_shorthand_notation_for_options_class(field: &Rc<InitializerField>, verifier: &mut Subverifier, options_class: &Thingy, missing: &mut HashSet<Thingy>) -> Result<(), DeferError> {
        ...
        match &name.0 {
            ...
            FieldName::NumericLiteral(_) => {
                verifier.verify_expression(value_exp, &default())?;
                verifier.add_verify_error(&name.1, FxDiagnosticKind::UnexpectedFieldName, diagarg![]);
            },
        }
        Ok(())
    }

    fn verify_initializer_shorthand(verifier: &mut Subverifier, id: &QualifiedIdentifier) -> Result<Option<Thingy>, DeferError> {
    ...
    fn resolve_instance_variable(verifier: &mut Subverifier, class: &Thingy, id: &QualifiedIdentifier) -> Result<Option<Thingy>, DeferError> {
        ...
        if variable.is_none() {
            verifier.add_verify_error(&id.location, FxDiagnosticKind::UnknownOptionForClass, diagarg![local_name]);
            return Ok(None);
        }
