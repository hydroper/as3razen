...
impl ArgumentsSubverifier {
    pub fn verify(verifier: &mut Subverifier, arguments: &Vec<Rc<Expression>>, signature: &Thingy) -> Result<(), VerifierArgumentsError> {
        let host = verifier.host.clone();
        let sig_params = signature.params();
        let mut sig_params = sig_params.iter();
        let mut rest_elem_type: Option<Thingy> = None;
